...
***************************************************************************/
extern Word16to8    syn_bias_7khz[DCT_LENGTH];
extern Word16to8    dither[DCT_LENGTH];
...
void dct_type_iv_s (Word16 *input,Word16 *output,Word16 dct_length)
{
    ...
    Word16   *buffer_a = share_mem2, *buffer_b = share_mem3, *buffer_c = share_mem_inout;
#else
    ...
    for (pairs_left = (Word16)(1 << (dct_length_log - 1));    pairs_left > 0;    pairs_left--)
    {
    ...
    for (set_count_log = (Word16)(dct_length_log - 2);    set_count_log >= 0;    set_count_log--)
    {
