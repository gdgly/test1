                           ...
                           Word16 mag_shift)             
{
...
#ifdef SUPPORT_STACK_SHAREMEM
    share_mem_inout = (Word16*)out_samples;
#endif
...
#ifdef SUPPORT_MEMORY_DEC_TABLE
    for (vals_left = half_dct_size,index=0;    vals_left > 0;    vals_left--,index++)
    {
        sum = 0L;
        move32();
        sum = L_mac(sum, (Word16)((*win_new++) - index), *--new_ptr);
        sum = L_mac(sum, (Word16)((*--win_old) - (158+(64*vals_left)/half_dct_size)), *old_ptr++);
    ...
    for (vals_left = half_dct_size,index=0;    vals_left > 0;    vals_left--,index++)
    {
        sum = 0L;
        move32();
        sum = L_mac(sum,(Word16)((*win_new++) - (158+(64*index)/half_dct_size)), *new_ptr++);
        sum = L_mac(sum,negate((Word16)((*--win_old) - vals_left)), *--old_ptr);
        *out_ptr++ = itu_round(L_shl_nocheck(sum,2));
        move16();
    }
