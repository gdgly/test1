...
// Write an unsigned value to the ini file
BOOL CIni::WriteUInt64(LPCTSTR lpSection, LPCTSTR lpKey, UINT64 nValue, int nBase) const
{
	TCHAR szValue[DEF_PROFILE_NUM_LEN + 1] = _T("");
...
UINT64 CIni::GetUInt64(LPCTSTR lpSection, LPCTSTR lpKey, UINT nDefault, int nBase) const
{
	TCHAR sz[DEF_PROFILE_NUM_LEN + 1] = _T("");
	GetString(lpSection, lpKey, sz, DEF_PROFILE_NUM_LEN);
...
void CIni::GetKeyNames(LPCTSTR lpSection, CStringArray *pArray) const
{
	...
	pArray->RemoveAll();
	const long LEN = GetKeyNames(lpSection, NULL, 0);
	LPTSTR psz = new TCHAR[LEN + 1];
...
// Convert an unsigned integer into string representation, based on its base
void CIni::__UInt64ToString(UINT64 nNumber, LPTSTR lpBuffer, int nBase) const
{
	switch (nBase)
	{
	case BASE_OCTAL:
		_stprintf(lpBuffer, _T("%llo"), nNumber);
		break;

	case BASE_HEXADECIMAL:
		_stprintf(lpBuffer, _T("%llX"), nNumber);
		break;

	default:
		_stprintf(lpBuffer, _T("%llu"), nNumber);
		break;
	}	
}
