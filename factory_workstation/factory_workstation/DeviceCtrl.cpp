...
void CDeviceCtrl::SetPlayTime(int sec)
{
	m_secPlay = sec;
}
...
int CDeviceCtrl::Recording(int mic, int sec, int bCloseEng)
{
	...
	CreateDirectory("audio\\check_sz\\record", NULL);
	sName.Format("audio\\check_sz\\record\\Mic%dRec_%04d%02d%02d_%02d%02d%02d.g722",mic,
...
int CDeviceCtrl::CrystalTrimmingRead(int &XtalFreqTrim, int &XtalLoadCap, int bCloseEng)
{
	...
	if ((ret = teConfigCacheReadItem(m_devHandle, "curator:XtalLoadCapacitance", msgbuf, &datalen)) != TE_OK) {
		...
		sText.Format("%s",msgbuf);
		if(sText.Find("0x") >= 0){
			XtalLoadCap = strtol(sText,NULL,16);
		}else{
			XtalLoadCap = _tstoi(sText);
		}
	}
	...
	if ((ret = teConfigCacheReadItem(m_devHandle, "curator:XtalFreqTrim", msgbuf, &datalen)) != TE_OK) {
		...
		sText.Format("%s",msgbuf);
		if(sText.Find("0x") >= 0){
			XtalFreqTrim = strtol(sText,NULL,16);
		}else{
			XtalFreqTrim = _tstoi(sText);
		}
	}
