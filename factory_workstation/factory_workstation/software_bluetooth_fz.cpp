// software_bluetooth_fz.cpp : 实现文件
//
...
// software_bluetooth_fz 对话框

IMPLEMENT_DYNAMIC(software_bluetooth_fz, CDialogEx)

software_bluetooth_fz::software_bluetooth_fz(CWnd* pParent /*=NULL*/)
	: CDialogEx(software_bluetooth_fz::IDD, pParent)
{

}

software_bluetooth_fz::~software_bluetooth_fz()
{
}

void software_bluetooth_fz::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(software_bluetooth_fz, CDialogEx)
	ON_MESSAGE(WM_USERMSG_MASSPRODUCT, &software_bluetooth_fz::OnMSGMassProduct)
	ON_BN_CLICKED(IDC_BUTTON13, &software_bluetooth_fz::OnBnClickedButton13)
	ON_EN_CHANGE(IDC_SET_SN, &software_bluetooth_fz::OnEnChangeSetSn)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_BUTTON2, &software_bluetooth_fz::OnBnClickedButton2)
	ON_BN_CLICKED(IDC_BUTTON1, &software_bluetooth_fz::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON17, &software_bluetooth_fz::OnBnClickedButton17)
	ON_BN_CLICKED(IDC_BUTTON14, &software_bluetooth_fz::OnBnClickedButton14)
	ON_BN_CLICKED(IDC_BUTTON15, &software_bluetooth_fz::OnBnClickedButton15)
	ON_BN_CLICKED(IDC_BUTTON20, &software_bluetooth_fz::OnBnClickedButton20)
END_MESSAGE_MAP()

void software_bluetooth_fz::AddEvent2List(CString sText)
{
...
INT software_bluetooth_fz::MesPass()   //每一块成品测完以后，点击PASS上传MES
{
...
LRESULT software_bluetooth_fz::OnMSGMassProduct(WPARAM wParam, LPARAM lParam)
{
	UINT a;
	int count;
	CString sText;
	switch((INT)wParam) {
		...
		case  WPARAM_MAC_SN:
			...
			break;
		case WPARAM_TFA9896_VALUE:
			if(m_iTFA8686val > (int)bluetooth_option_instance.g_cali_min_value_fz && 
				m_iTFA8686val < (int)bluetooth_option_instance.g_cali_max_value_fz){//6500 上下 20% 5200-7800
				sText.Format("OK %d", m_iTFA8686val);
				m_iTFAVal.SetWindowTextA(sText);
				calibration_flag = 1;
				AddEvent2List("SmartCodec校准成功");
				if(m_hLog_calibration_file.m_hFile != INVALID_HANDLE_VALUE) {
					CTime tNow = CTime::GetCurrentTime();
					m_hLog_calibration_file.Write( m_setSN, m_setSN.GetLength() );
					sText.Format("        ");
					m_hLog_calibration_file.Write(sText, sText.GetLength());
					sText.Format("%04d.%02d.%02d-%dh:%dm:%ds",	tNow.GetYear(), tNow.GetMonth(), 
						tNow.GetDay(), tNow.GetHour(), tNow.GetMinute(), tNow.GetSecond());
					m_hLog_calibration_file.Write(sText, sText.GetLength());
					sText.Format("        ");
					m_hLog_calibration_file.Write(sText, sText.GetLength());
					sText.Format("OK %d", m_iTFA8686val);
					m_hLog_calibration_file.Write(sText, sText.GetLength());
					m_hLog_calibration_file.Write("\r\n", 2); m_hLog_calibration_file.Flush();
				}
			}else{
				calibration_flag = 0;
				sText.Format("ERR %d", m_iTFA8686val);
				m_iTFAVal.SetWindowTextA(sText);
				AfxMessageBox("SmartCodec校准出错");
				AddEvent2List("SmartCodec校准出错");
			}
			break;
		...
		case WPARAM_THREAD_EXITED:
			...
			if(aud_right_db >= (int)bluetooth_option_instance.g_db_value_fz && 
				aud_left_db >= (int)bluetooth_option_instance.g_db_value_fz && 
				abs(aud_right_db - aud_left_db) <= (int)bluetooth_option_instance.g_sub_value_fz &&
				calibration_flag == 1){
...
int getAvgPcmDB(INT16  *pcmdata,  long nmemb)
{  
	int db = 0;  
	double sum = 0; 
	...
	//db =(int) 20*log10((sum /nmemb));
	db = ( int )floor( 20*log10((sum /nmemb)) );
	return db;  
} 


int software_bluetooth_fz::WorkerThreadProc(void *param)
{
	CString sText;
	int timeout = 0;
	...
	CTime tNow,tSend;
	CTimeSpan timeSpan;
	software_bluetooth_fz *p = (software_bluetooth_fz*)param;
...
ReParse:
	...
	tNow = CTime::GetCurrentTime();
	sText.Format("audio\\bluetooth_fz\\Aud_%s_%02d%02d%02d", p->m_sSN,
...
void software_bluetooth_fz::get_param(struct glob_param *g_p)
{
...
void software_bluetooth_fz::dlg_pram_init()
{
	pass_green_color = 1;
	calibration_flag = 0;
...
void software_bluetooth_fz::OnBnClickedButton13()
{
	// TODO: 在此添加控件通知处理程序代码
	dlg_pram_init();
...
void software_bluetooth_fz::OnEnChangeSetSn()
{
	...
	if(sText.GetLength() == 18){//v2 sn eg:A01A0A180801000001
		if( sText.Find("-") < 0 ){
			if(CheckSNv2(sText, general_option_instance.g_sn_prompt, 
				...
				hThread   = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_bluetooth_fz::WorkerThreadProc, this, 0, &idThread);
				idThread  = idThread;
			}
		}
	}
...
BOOL software_bluetooth_fz::OnInitDialog()
{
	CDialogEx::OnInitDialog();
	ModifyStyleEx(WS_EX_TOOLWINDOW, WS_EX_APPWINDOW);
	// TODO:  在此添加额外的初始化
	CreateDirectory("audio", NULL);
	CreateDirectory("audio\\bluetooth_fz", NULL);
...
HBRUSH software_bluetooth_fz::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void software_bluetooth_fz::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	CString temp;
	m_sFileName.Empty();
...
void software_bluetooth_fz::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
	if(m_sFileName.Find("g722") >= 0) {
...
void software_bluetooth_fz::OnBnClickedButton17()
{
	// TODO: 在此添加控件通知处理程序代码
...
void software_bluetooth_fz::OnBnClickedButton14()
{
	// TODO: 在此添加控件通知处理程序代码
	CString sText;
...
void software_bluetooth_fz::OnBnClickedButton15()
{
	// TODO: 在此添加控件通知处理程序代码
...
BOOL software_bluetooth_fz::PreTranslateMessage(MSG* pMsg)
{
...
void software_bluetooth_fz::OnBnClickedButton20()
{
	// TODO: 在此添加控件通知处理程序代码
