...
// software_check_jn 对话框

IMPLEMENT_DYNAMIC(software_check_jn, CDialogEx)

software_check_jn::software_check_jn(CWnd* pParent /*=NULL*/)
	: CDialogEx(software_check_jn::IDD, pParent)
{

}

software_check_jn::~software_check_jn()
{
}

void software_check_jn::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STC_88L24, m_btn88L24);
...
BEGIN_MESSAGE_MAP(software_check_jn, CDialogEx)
	ON_MESSAGE(WM_USERMSG_MASSPRODUCT, &software_check_jn::OnMSGMassProduct)
	ON_EN_CHANGE(IDC_EDIT2, &software_check_jn::OnEnChangeEdit2)
	ON_BN_CLICKED(IDC_BUTTON13, &software_check_jn::OnBnClickedButton13)
	ON_WM_CTLCOLOR()
	ON_BN_CLICKED(IDC_RADIO_MUSIC1, &software_check_jn::OnBnClickedRadioMusic1)
	ON_BN_CLICKED(IDC_RADIO_MUSIC2, &software_check_jn::OnBnClickedRadioMusic2)
	ON_BN_CLICKED(IDC_RADIO_MUSIC3, &software_check_jn::OnBnClickedRadioMusic3)
	ON_BN_CLICKED(IDC_RADIO_MUSIC4, &software_check_jn::OnBnClickedRadioMusic4)
	ON_BN_CLICKED(IDC_RADIO_MUSIC5, &software_check_jn::OnBnClickedRadioMusic5)
	ON_BN_CLICKED(IDC_RADIO_MUSIC6, &software_check_jn::OnBnClickedRadioMusic6)
END_MESSAGE_MAP()

void software_check_jn::AddEvent2List(CString sText)
{
...
void software_check_jn::CheckNewStatus(MPIPtr mpi, MsgInfo *pmsg)
{
	BOOL snOK = FALSE;
	CString sText;
	...
	case BIT_KEY_TOUCH_DOWN:   
		...
		break;
	case BIT_NAU88L24:                   // NAU88L24 OK
		m_btn88L24.SetColor(STATIC_WND_SUCCESS);
		break;
	...
	case BIT_SN_OK:
		...
		if(mpi->burncheck_option_instance.g_if_check_version_str){
			AddEvent2List("sn字串检测正确");
		}
		break;
	...
	case BIT_FIRMVER_OK:
		...
		break;
	case BIT_LANGUAGES_STR:
		AddEvent2List("语种字串检测正确");
		break;
	}
}

LRESULT software_check_jn::OnMSGMassProduct(WPARAM wParam, LPARAM lParam)
{
	UINT a;
	CString sText;
...
int software_check_jn::WorkerThreadProc(void *param)
{
	...
	UINT first_pass_status = BIT_START_INIT_JN;
	if(mpi->burncheck_option_instance.g_if_check_version_str)
		first_pass_status |= (1<<BIT_LANGUAGES_STR);
	while(0 == mpi->doExit) {
		...
		if(mpi->burncheck_option_instance.g_current_test){
			...
			mpi->current = pComCheck->GetCurrent();                   //获取充电电流
			if(mpi->burncheck_option_instance.g_sleep_current_test){//如果勾选休眠电流检测获取休眠电流
		...
		switch(mpi->runStatus) {
			case RUNST_CHECK_INIT:                   //启动校验板子的状态，由板子打印的数据获得   7
						...
						if((bitStat & (1<<i)) && (!(mpi->bitStatus & (1<<i)))) {
							...
							if(BIT_SN_OK == i) {
								...
								if(mpi->burncheck_option_instance.g_if_check_version_str){
									...
									memset(c_buf, 0, 10);
									memcpy(c_buf, mpi->sNoSN, 4);
									CString sn4b_buf;
									sn4b_buf.Format("%s", (char*)c_buf);
									if(sn4b_buf != mpi->burncheck_option_instance.g_out_sn_str){
										AfxMessageBox("sn号与接收字串不一致，检查是否设置正确！");  
										ChangeNewStatuc(mpi, 100, __LINE__);										
									}
								}
							}
							...
							if(BIT_LANGUAGES_STR == i) {
								mpi->language = pComBoard->Getlanguage();
								if(mpi->language != mpi->burncheck_option_instance.g_out_languages_str){
										AfxMessageBox("语种字串不一致，检查是否设置正确！");  
										ChangeNewStatuc(mpi, 100, __LINE__);									
								}
							}
				...
				if(mpi->bitStatus == first_pass_status) {         // 检测到启动后，每三秒发一次key_dbg命令，防止命令没有正确接收
				...
				//if((mpi->bitStatus & BIT_KEY_CHECKED_JN) == BIT_KEY_CHECKED_JN) {               /////开始音频文件的播放   127     mpi->bitStatus  8个显示颜色的标志位
				if( (mpi->bitStatus & (first_pass_status | (1<<BIT_KEY_TOUCH_UP) | (1<<BIT_KEY_TOUCH_DOWN) | (1<<BIT_KEY_BOARD))) == 
					(first_pass_status | (1<<BIT_KEY_TOUCH_UP) | (1<<BIT_KEY_TOUCH_DOWN) | (1<<BIT_KEY_BOARD)) ){
			...
			case RUNST_CHECK_AUDIO:
				bitStat = pComBoard->GetStatus();
				if((mpi->bitStatus&first_pass_status) == first_pass_status) {   // 启动完成
...
int software_check_jn::PlaysoundThreadProc(void *param)
{
	...
	while(0 == mpi->doExit) {
		if(mpi->iAudSetloops != mpi->iAudPlayloops) {
			...
			::waveOutSetVolume(0, 0xFFFFFFFF);          // LEFT
			...
			::waveOutSetVolume(0, 0xFFFFFFFF);      // RIGHT
...
void software_check_jn::OnEnChangeEdit2()
{
...
void software_check_jn::SetAllItemRestart(void)
{
	CString sText;
	...
	m_btn88L24.SetColor(STATIC_WND_DISABLE);
...
// software_check_jn 消息处理程序

void software_check_jn::OnBnClickedButton13()
{
	// TODO: 在此添加控件通知处理程序代码
	MPIPtr mpi = (MPIPtr)&m_MPInfo;
	SetAllItemRestart();
...
void software_check_jn::get_param(struct glob_param *g_p)
{
...
BOOL software_check_jn::OnInitDialog()
{
	...
	mpi->hThread   = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_jn::WorkerThreadProc, mpi, 0, &idThread);
	mpi->idThread  = idThread;

	mpi->hAudThread   = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_jn::PlaysoundThreadProc, mpi, 0, &idThread);
...
HBRUSH software_check_jn::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void software_check_jn::CheckUserSelectAud(int iSelectNo)
{
	MPIPtr mpi = (MPIPtr)&m_MPInfo;
	mpi->audSelect = iSelectNo;
}

void software_check_jn::OnBnClickedRadioMusic1()
{
	// TODO: 在此添加控件通知处理程序代码
...
void software_check_jn::OnBnClickedRadioMusic2()
{
	// TODO: 在此添加控件通知处理程序代码
...
void software_check_jn::OnBnClickedRadioMusic3()
{
	// TODO: 在此添加控件通知处理程序代码
...
void software_check_jn::OnBnClickedRadioMusic4()
{
	// TODO: 在此添加控件通知处理程序代码
...
void software_check_jn::OnBnClickedRadioMusic5()
{
	// TODO: 在此添加控件通知处理程序代码
...
void software_check_jn::OnBnClickedRadioMusic6()
{
	// TODO: 在此添加控件通知处理程序代码
...
BOOL software_check_jn::PreTranslateMessage(MSG* pMsg)
{
