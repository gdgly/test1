...
IMPLEMENT_DYNAMIC(software_check_by, CDialogEx)

software_check_by::software_check_by(CWnd* pParent /*=NULL*/)
	: CDialogEx(software_check_by::IDD, pParent)
{

}

software_check_by::~software_check_by()
{
}

void software_check_by::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(software_check_by, CDialogEx)
	ON_MESSAGE(WM_USERMSG_MASSPRODUCT, &software_check_by::OnMSGMassProduct)
	ON_BN_CLICKED(IDC_BUTTON13, &software_check_by::OnBnClickedButton13)
	ON_EN_CHANGE(IDC_EDIT2, &software_check_by::OnEnChangeEdit2)
	ON_WM_CTLCOLOR()
	...
	ON_BN_CLICKED(IDC_BUTTON21, &software_check_by::OnBnClickedButton21)
END_MESSAGE_MAP()

void software_check_by::AddEvent2List(CString sText)
{
...
void software_check_by::get_param(struct glob_param *g_p)
{
...
// software_check_by 消息处理程序
void software_check_by::init_err_process(unsigned int flag)
{
	CString sText;
...
void software_check_by::init_part_process(unsigned int flag)
{
	CString sText;
	if(flag & (1<<BY_CHECK_INIT_FLAG_SN)){
...
LRESULT software_check_by::OnMSGMassProduct(WPARAM wParam, LPARAM lParam)
{
	float time_ms;
	CString sText;
...
int software_check_by::WorkerThreadProc(void *param)
{
	...
	CTime tLastPlay;
	software_check_by *p = (software_check_by*)param;
	...
	if(p->check_by_option_instance.g_current_test){
		if(FALSE == pComCheck->InitComm(p->check_by_option_instance.g_currentcom_port, DEIGHT, SONE, PNONE, 
			...
			CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::COMThreadProc, param, 0, NULL);
		}
	...
	while(1){
		...
		switch (status){
			...
			case BY_CHECK_CREATE_INIT:
				CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::create_app_sn_init, param, 0, NULL);
				CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::create_app_wifi_init, param, 0, NULL);
				CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::create_app_bt_init, param, 0, NULL);
				CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::create_app_start_play_init, param, 0, NULL);
				msg_p = new msg_by_check_info;
			...
			case BY_CHECK_CHECK_INIT:
				...
				if(p->init_err_flag){
					...
					::PostMessage(p->m_hWnd, WM_USERMSG_MASSPRODUCT, 0, (LPARAM)msg_p);
					goto out;
				}/*else if(p->init_flag != init_part_flag){
					msg_p = new msg_by_check_info;
					msg_p->msg_type = BY_CHECK_MSG_TYPE_INIT_PART;
					msg_p->data1 = p->init_flag - init_part_flag;
					::PostMessage(p->m_hWnd, WM_USERMSG_MASSPRODUCT, 0, (LPARAM)msg_p);	
					init_part_flag = p->init_flag;
			...
			case BY_CHECK_CHECK_KEY:
				CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::create_app_key_init, param, 0, NULL);
				pComCheck->SendComand(PATH_TEST_R);
				cmd_param = " push firmware/check/1KHZ4s.wav /tmp";//push wav放这里节省时间
				response = p->ExecuteCmd(cmd_str, cmd_param);
				if(p->if_response_err(response)){
					msg_p = new msg_by_check_info;
					msg_p->msg_type = BY_CHECK_MSG_TYPE_INIT_ERR;
			...
			case BY_CHECK_CHECK_AUDIO:
				pComCheck->SendComand(NOPATH_TEST_R);//不过测试电阻，使电流板能供电正常。
				Sleep(20);
				CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::create_app_aplay_init, param, 0, NULL);
				msg_p = new msg_by_check_info;
...
int software_check_by::COMThreadProc(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CContrlComm *pComCheck = &(p->comInfo);
	//msg_by_check_info *msg_p;
...
DWORD software_check_by::MicCallBack(HWAVEIN hWaveIn, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2)
{
    // 所有的这些录音缓冲区都是由录音函数自动触发的，开发这不需要自己触发
	software_check_by *pwnd = (software_check_by*)dwInstance;     // 表示录音的窗体
...
int software_check_by::pc_record()
{
	msg_by_check_info *msg_p;
	...
	CString sText;
	CTime tNow = CTime::GetCurrentTime();
	sText.Format("audio\\check_by\\Aud_%s_%02d%02d%02d.pcm", sn_read,
...
int software_check_by::create_app_aplay_init(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
...
int software_check_by::PlaysoundThreadProc(void *param)
{
...
TCHAR* software_check_by::StringToChar(CString& str)
{
...
CString software_check_by::ExecuteCmd(CString cmd_name, CString cmd_param)
{
...
int software_check_by::process_response_devices(CString response)
{
	CString cut_str;
...
int software_check_by::create_app_sn_init(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
	CString cmd_param;
	cmd_param = " push firmware/check/sn_init /tmp";
	response = p->ExecuteCmd(cmd_str, cmd_param);
	if(p->if_response_err(response)){
		return -1;
	}
...
int software_check_by::create_app_wifi_init(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
	CString cmd_param;
	cmd_param = " push firmware/check/wifi_init /tmp";
	response = p->ExecuteCmd(cmd_str, cmd_param);
	if(p->if_response_err(response)){
		return -1;
	}
	cmd_param = " shell chmod +x /tmp/wifi_init";
	response = p->ExecuteCmd(cmd_str, cmd_param);
	if(p->if_response_err(response)){
		return -1;
	}
	cmd_param = " shell /tmp/wifi_init";
	response = p->ExecuteCmd(cmd_str, cmd_param);
	if(p->if_response_err(response)){
		return -1;
	}

	int n=response.Find("wifi_init OK");
	if(n<0){
...
int software_check_by::create_app_bt_init(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
	CString cmd_param;
	cmd_param = " push firmware/check/bt_init /tmp";
	response = p->ExecuteCmd(cmd_str, cmd_param);
	if(p->if_response_err(response)){
		return -1;
	}
	cmd_param = " shell chmod +x /tmp/bt_init";
	response = p->ExecuteCmd(cmd_str, cmd_param);
	if(p->if_response_err(response)){
		return -1;
	}
	cmd_param = " shell /tmp/bt_init";
	response = p->ExecuteCmd(cmd_str, cmd_param);
	if(p->if_response_err(response)){
		return -1;
	}

	int n=response.Find("bt_init OK");
	if(n<0){
...
int software_check_by::create_app_start_play_init(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
	...
	cmd_param = " push firmware/check/warning.wav /tmp";
	response = p->ExecuteCmd(cmd_str, cmd_param);
	if(p->if_response_err(response)){
		return -1;
	}
...
int software_check_by::create_app_key_init(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
...
int software_check_by::key_process(CString response)
{
	msg_by_check_info *msg_p;
...
CString software_check_by::ExecuteCmd_keyprocess(CString cmd_name, CString cmd_param)
{
	SECURITY_ATTRIBUTES sa;
    HANDLE hRead,hWrite;
...
int software_check_by::key_init()
{
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
	CString cmd_param;
...
int software_check_by::create_app_start_play(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
...
int software_check_by::if_response_err(CString response)
{
...
int software_check_by::need_file_ifexit()
{
	if(PathFileExists("thirdapp\\adb\\adb.exe") != TRUE){
		AfxMessageBox("错误：程序路径下未找到adb.exe");
		return 1;
	}
	if(PathFileExists("firmware\\check\\sn_init") != TRUE){
	...
	if(PathFileExists("firmware\\check\\start_play") != TRUE){
		AfxMessageBox("错误：程序路径下未找到start_play");
		return 1;
	}
	if(PathFileExists("firmware\\check\\button_check") != TRUE){
		AfxMessageBox("错误：程序路径下未找到button_check");
		return 1;
	}
	if(PathFileExists("firmware\\check\\1KHZ4s.wav") != TRUE){
...
void software_check_by::SetAllItemRestart(void)
{
	CString sText;
...
BOOL software_check_by::OnInitDialog()
{
	CDialogEx::OnInitDialog();
	ModifyStyleEx(WS_EX_TOOLWINDOW, WS_EX_APPWINDOW);
	// TODO:  在此添加额外的初始化
	CString dir_path;
	dir_path = LOG_PATH_CHECK_by;
	...
	CreateDirectory("audio\\check_by", 0);
...
void software_check_by::OnBnClickedButton13()
{
	// TODO: 在此添加控件通知处理程序代码
	SetAllItemRestart();
}
int software_check_by::kill_adbd(void *param)
{
	software_check_by *p = (software_check_by*)param;
	CString response;
	CString cmd_str = "thirdapp\\adb\\adb.exe";
...
void software_check_by::OnBnClickedButton21()
{
	// TODO: 在此添加控件通知处理程序代码
	CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::kill_adbd, this, 0, NULL);
}
...
void software_check_by::OnEnChangeEdit2()
{
	...
	if(sText.GetLength() == 18){//v2 sn eg:A01A0A180801000001
		if( sText.Find("-") < 0 ){
			if(CheckSNv2(sText, general_option_instance.g_sn_prompt, 
				...
				CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::WorkerThreadProc, this, 0, NULL);
...
BOOL software_check_by::PreTranslateMessage(MSG* pMsg)
{
...
HBRUSH software_check_by::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
//void software_check_by::OnBnClickedRadioMusic1()
//{
//	// TODO: 在此添加控件通知处理程序代码
//	audSelect = 0;
//}
//
//void software_check_by::OnBnClickedRadioMusic2()
//{
//	// TODO: 在此添加控件通知处理程序代码
//	audSelect = 1;
//}
//
//void software_check_by::OnBnClickedRadioMusic3()
//{
//	// TODO: 在此添加控件通知处理程序代码
//	audSelect = 2;
//}
//
//void software_check_by::OnBnClickedRadioMusic4()
//{
//	// TODO: 在此添加控件通知处理程序代码
//	audSelect = 3;
//}
//
//void software_check_by::OnBnClickedRadioMusic5()
//{
//	// TODO: 在此添加控件通知处理程序代码
//	audSelect = 4;
//}
//
//void software_check_by::OnBnClickedRadioMusic6()
//{
//	// TODO: 在此添加控件通知处理程序代码
//	audSelect = 5;
//}
			...
			//	p->audNo = rand() % 6;
			//	CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::PlaysoundThreadProc, param, 0, NULL);
			//	CreateThread(0, 0, (LPTHREAD_START_ROUTINE)software_check_by::create_app_start_play, param, 0, NULL);
			//	msg_p = new msg_by_check_info;
			//	msg_p->msg_type = BY_CHECK_MSG_TYPE_START_AUDIO;
			//	::PostMessage(p->m_hWnd, WM_USERMSG_MASSPRODUCT, 0, (LPARAM)msg_p);
			//	status = BY_CHECK_CHECK_WAITAUDIO;
			//	tLastPlay = CTime::GetCurrentTime();
			//	break;
			//case BY_CHECK_CHECK_WAITAUDIO:
			//	if(p->init_err_flag){
			//		msg_p = new msg_by_check_info;
			...
			//		goto out;
			//	}
			//	if(p->audSelect != p->audNo) {//点的不对或者没点
			//		if( (tNow.GetTime() - tLastPlay.GetTime()) >= 4 ){
			//			p->audNo = rand() % 6;
			//			tLastPlay = CTime::GetCurrentTime();
			//		}
			//	}else{
			//		p->PlaysoundThreadProc_exit = 1;
			//		cmd_param = " shell killall start_play";
			//		response = p->ExecuteCmd(cmd_str, cmd_param);
			//		msg_p = new msg_by_check_info;
			//		msg_p->msg_type = BY_CHECK_MSG_TYPE_AUDIO_SUC;
			//		::PostMessage(p->m_hWnd, WM_USERMSG_MASSPRODUCT, 0, (LPARAM)msg_p);
